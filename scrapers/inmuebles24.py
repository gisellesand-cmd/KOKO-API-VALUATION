...
# Respects Inmuebles24 robots.txt; MIN_DELAY_SECONDS enforces a polite crawl rate.
...
class Inmuebles24Scraper(BaseScraper):
    ...
    def _build_search_url(
        ...
        if page <= 1:
            return f"{self.BASE_URL}/{pt}-en-{op}-en-{location}.html"
        return f"{self.BASE_URL}/{pt}-en-{op}-en-{location}-pagina-{page}.html"
    ...
    def _parse_listing_card(
        ...
        listing_id = (
            ...
            or card.attributes.get("data-posting-id")
        )
        if not listing_id:
            self.logger.info(
        ...
        features_text = " ".join(
            n.text(strip=True)
            for n in card.css("ul.card-points li, ul.section-icon-features li, [class*='feature']")
        )
        ...
        combined_text = " ".join(filter(None, [title or "", features_text, address or ""]))
